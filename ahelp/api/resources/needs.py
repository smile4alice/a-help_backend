...
class NeedsResource(Resource):
    ...
    get:
        tags:
            - api
        summary: Get data from blocks of needs
        description: Get data from the needs blocks, with the possibility to choose a specific language
        parameters:
            - in: path
              name: language
              schema:
    ...
    def get(self, language=None):
        try:
            ...
            if int(limit) > 0:
                needs_data = NeedsModel.query.filter(NeedsModel.active == True).order_by(NeedsModel.creation_date.asc()).limit(limit).all()
            else:
                needs_data = NeedsModel.query.filter(NeedsModel.active == True).order_by(NeedsModel.creation_date.asc()).all()
            if not needs_data:
